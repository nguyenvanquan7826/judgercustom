...
<?php
	if (isset($success) && $success) {
		if (!is_file("$accountDir/tmp.xml")) {
			$fi = fopen("$accountDir/account.xml",'r');
			$fo = fopen("$accountDir/tmp.xml",'w');
			while (!feof($fi)) {
				$str = fgets($fi);
			...
			fclose($fi);
			fclose($fo);
			copy("$accountDir/tmp.xml","$accountDir/account.xml");
			unlink("$accountDir/tmp.xml");
